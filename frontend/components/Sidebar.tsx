  ...
  return (
    <aside className="w-68 bg-white p-5 shadow-md border">
        <nav className="space-y-2">
          <Link href="/dashboard/profile"><SidebarItem icon={<div className="bg-blue-500 text-white rounded-full w-10 h-10 flex items-center justify-center text-lg font-bold">{name?.substring(0, 2).toUpperCase() || 'WMW'}</div>} label={<span className="text-gray-700 font-semibold">{name}</span>} active={activeItem === ""} onClick={handleSidebarClick} /></Link>
          {role === "admin"? 
          <div className='pl-8'>
