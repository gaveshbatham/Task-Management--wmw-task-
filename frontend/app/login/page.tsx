...
export default function Login() {
  ...
  async function handleFormSubmit(formData: FormData) {
    formData.set("role", selectedRole); // Ensure role is included
    ...
    <div className="flex flex-col gap-12 bg-[#f3f2f2] min-h-screen">
      ...
      <div className="flex flex-col items-center justify-center max-w-7xl mx-auto my-auto">
        <div className="w-full min-w-sm max-w-md p-8 bg-white rounded-2xl shadow-lg">
          ...
          <form className="space-y-4 flex flex-col items-center" action={handleFormSubmit}>
            {/* Role Selection */}
            <div className="flex border border-gray-300 rounded-lg overflow-hidden mb-12 w-[9rem] self-center">
              {(["admin", "user"] as Role[]).map((role) => (
                <label
                  key={role}
                  className={`px-4 py-2 cursor-pointer ${selectedRole === role ? "bg-blue-600 text-white" : "bg-white text-gray-600"}`}
                >
                  <input
                    type="radio"
                    name="role"
            ...
            <div className="w-[98%]">
              ...
              <div className="relative">
                <Input type={showPassword ? "text" : "password"} placeholder="Enter password here" name="password" required />
            ...
            {/* Submit Button */}
            <Button className="w-full bg-blue-500" type="submit" disabled={pending}>
